class KitsController < ApplicationController
  ...
  def check
    ...
    if params[:check]
      ...
      kit_id = @kit.id
      @kit = {}
      @kit[:ns] = Kit.connection.exec_query("select * from Ns where tree_id = #{tree_id}").to_a
      @kit[:segments] = Kit.connection.exec_query("select * from Segments where tree_id = #{tree_id}").to_a
      @kit[:sections] = Kit.connection.exec_query("select * from Sections where tree_id = #{tree_id}").to_a
      @kit[:segmentos] = Kit.connection.exec_query("select * from Segmentos where tree_id = #{tree_id}").to_a
      @kit[:sectionos] = Kit.connection.exec_query("select * from Sectionos where tree_id = #{tree_id}").to_a
      KitCheck.create(kit_id: kit_id, check: params[:check])
    end
    respond_to do |format|
      format.html
      format.json { render json: @kit }
  ...
  def redactions
    respond_to do |format|
      format.json do
        x = Redaction.where( docid: params[:wav] )
        if params[:get]
          x = x.pluck(:kit_id)
          x = Kit.where(id: x).pluck(:tree_id)
          x = Node.where(tree_id: x, node_value_id: 1).pluck(:parent_id)
          x = Node.where(parent_id: x, name: 'Segment').pluck(:node_value_id)
          x = NodeValue.where(id: x).map { |x| { beg: x.begr.to_f, end: x.endr.to_f } }
        else
          x = x.count
        end
        render json: (
  ...
  def get_broken_comment(kit)
    kit.state == 'broken' ? kit.broken_comment : "n/a"
  end

class NodeClassesController < ApplicationController
  ...
  def edit
    ...
    respond_to do |format|
      format.html do
        html = render_to_string(partial: 'editt', locals: { nc: })
        render json: { html: }
      end
    end
  end
  ...
  def create
    ...
    return if has_restricted_name(@nc.name, @class_def.name, 'Add')
  ...
  def update
    ...
    children = node_supertypes.include?(parent_name) ? child_string_from_params(params, parent_name) : nil
    ...
    value['label'] = params[:child_text] if %w[Label EntryLabel Entry Ref].include? parent_name
    # value['template'] = params[:node_class][:template] if params[:node_class][:template]
    if parent_name == 'Button'
      value['label'] = params[:child_text]
      value['message'] = params[:child_action]
    end
    ...
    puts children
    unless childnames_unique(children)
      puts 'not unique'
      return
    end
    ...
    if nc
      ...
      wrap = []
      nc.each do |k, v|
        next unless k =~ /wrap-(\d+)-(\d)/

        ki = ::Regexp.last_match(1).to_i
        kj = ::Regexp.last_match(2).to_i
        wrap[ki] ||= []
        wrap[ki][kj] = v
      end
      ...
      @node_class.constraints['messages'] ||= []
      @node_class.constraints['messages'] = @node_class.constraints['messages'].select do |x|
        (!x.has_key? 'add') and
          (!x.has_key? 'delete')
      end
      @node_class.constraints['attach'] ||= []
      ...
      if params[:constraints_map]
        ...
        params[:constraints_map].split(/[\r\n]+/).each do |line|
          m[::Regexp.last_match(1)] = ::Regexp.last_match(2) if line =~ /^\s*(\w+)\s*:(.+)/
        end

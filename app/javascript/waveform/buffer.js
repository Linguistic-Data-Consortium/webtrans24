import { getp_wav } from 'https://cdn.jsdelivr.net/gh/Linguistic-Data-Consortium/ldcjs@0.0.3/src/getp.js'
import { ParseB } from './parse_b';
import { set_urls } from './aws_helper'
import { decode_audio_data } from 'ldcjs/src/audio/main';
let last_audio;
let last_buffer;
function convert_seconds_to_samples(seconds, wave_buffer_sample_rate){
  return Math.floor(seconds * wave_buffer_sample_rate);
}
function get_audio_buffer_for_waveform(w, audio, wave_display_offset, wave_display_length, wave_canvas_width){
  ...
  return get_audio_buffer(audio).then( (buffer) => {
    ...
    w.wave_display_length_in_samples = convert_seconds_to_samples(wave_display_length, w.wave_buffer_sample_rate);
    w.wave_display_offset_in_samples = convert_seconds_to_samples(wave_display_offset, w.wave_buffer_sample_rate);
    w.samples_per_pixel = w.wave_display_length_in_samples / wave_canvas_width;
...
function get_audio_buffer_info_for_waveform(w, wav){
  let p = get_audio_buffer_info(wav);
  return p.then( (info) => audiof(info) );
...
function audiof(data){
  ...
  w.component.set_times_then_draw(w.play_head, l);
}
export { get_audio_buffer_for_waveform, get_audio_buffer, get_audio_buffer_info_for_waveform }

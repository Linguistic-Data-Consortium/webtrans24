<script>
    ...
    let sort_key;
    function sort3(a, b){
        return sort1(a[1][sort_key].replace(/<.+?>/g, ''), b[1][sort_key].replace(/<.+?>/g, ''));
    }
    function sort2(a, b){
        return sort1(a[1][sort_key], b[1][sort_key]);
    }
    function sort1(a, b){
    ...
    let matched = new Map();
    function match(k, x){
        if(x._i == 0) matched = new Map();
        let b = match3(x);
        if(b) matched.set(k, x);
        return b;
    }
    function match3(x){
...
<div>
    ...
    <!--                             <div>
     ...
     -->                            <div>Preview</div>
            ...
            {/if}
            <div class="float-right">
                {matched.size} matched <input class="form-control" bind:value={filter} placeholder="filter">
            </div>
            {#if use_menu_filters}
    ...
    <div class={`overflow-y-auto ${name}`} style="height: {height}px">
        ...
        {#each [...rows] as [k, x]}
            {#if match(k, x) && filter.length > -1 && filters}
                <div class="
                    d-table col-12
                    {k == selected ? 'color-bg-success': '' }
                    {k == text_red ? 'color-text-danger': '' }
                    {x._i % 2 ? 'odd' : ''}
                "
                on:click={click_select} data-key={k}>
                    {#each columns as y, i}

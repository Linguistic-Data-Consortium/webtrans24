...
class KitType < ActiveRecord::Base
  ...
  def new_root_helper(kit)
    ...
    km.update(value: kit.class_def.name) if km.value.nil?
  ...
  def new_root(kit, task_user)
    ...
    if class_def.class == XlassDef
      create_with_json(task_user, kit, @tree, json)
      return @tree
    else
      json = json.to_json
      create_with_json(task_user, kit, @tree, json)
    ...
    if constraints['init_with_journal']
      constraints['init_with_journal'].split(',').each do |x|
        ...
        if b.nil? or kit.source[:uid].include? b
          ...
          if j
            create_with_json(task_user, kit, @tree, j.json)
          end
        end
      end
    end
  ...
  def new_root2(kit:, task_user:, journals:)
    ...
    return @tree if class_def.class == XlassDef

    @tree.tree

  end

  def create_with_json(task_user, kit, tree, json)
    class_def = kit.class_def
    if class_def.class == XlassDef
      ...
      AnnotationJournal.create(
        ...
        tree_id: tree.id,
        json: json,
        grammar: class_def.inverted_grammar.to_yaml
      )
    end
  end

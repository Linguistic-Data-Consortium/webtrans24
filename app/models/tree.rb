class Tree < ActiveRecord::Base
  ...
  def xlass_def
    XlassDef.where(id: class_def_id).first
  end
  ...
  def delete_xodes
    XnnotationJournal.where(tree_id: id).delete_all
    Xnnotation.where(tree_id: id).delete_all
    Xode.where(tree_id: id).delete_all
  end

  def create_xodes
    map = {}
    # nv = {}
    Node.includes(:node_value).where(tree_id: id).each do |x|
      map[x.iid] = x;
      # nv[x.node_value_id] = x.node_value.value
    end
    map.each do |k, v|
      case class_def_id
      when 586
        if v.name.in? %w[ SegmentListItem NormListItem ]
          children = v.children_nodes.map { |x| [ map[x].iid, map[x].name ].join(':') }.select { |x| x !~ /List\z/ }
              ...
              messages: [
                {
                  node: 0,
                  message: 'add',
                  value: { value: ( [ "#{k}:#{v.name}" ] + children ).join(',') }
                },
              ] + children.map { |y|
                a, b = y.split ':'
                x = map[a]
                {
                  node: x.iid.to_i,
                  message: 'change',
                  value: { value: YAML.load(x.node_value.value)['value'] }
                }
              }
            },
            user_id: v.user_id,

...
if File.exists? "tmp/init_db_for_#{ENV['POSTGRES_DB']}"
  ...
  if $ua['class_defs']
    $ua['class_defs'].each do |x|
      ...
      xd = XlassDef.where(name: class_def_name, original_id: cd.id).first_or_create
      cd.update(css: x['css'])
      x['node_classes'].each do |x|
        if x['parent'] == 'NodeList'
          nl = NodeList.where(class_def_id: xd.id, name: x['name']).first_or_create
          nl.update(children: x['children'])
          next
        end
